...
if exist "%AGENT_HOME%\lib\submit.jar" goto okSubmitJar
echo "Cannot locate submit jar"
goto exitBatch

:okSubmitJar
set AGENT_SUBMIT_JAR=%AGENT_HOME%\lib\submit.jar

@rem Execute Java Program
java %JAVA_OPTS% -classpath "%AGENT_JAR%;%AGENT_SUBMIT_JAR%" com.hengtiansoft.peg.byteagent.submit.Submit %*
